...
class _EfficientKAN(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        super(_EfficientKAN, self).__init__()
        self.grid_size = grid_size
        self.spline_order = spline_order
...
class EfficientKAN(nn.Module):
    ...
    def __init__(self, layers: Sequence[int], **kwargs):
        ...
        for layer in getattr(self._model, "layers", []):
            if hasattr(layer, "enable_standalone_scale_spline"):
                layer.enable_standalone_scale_spline = True
    ...
    def regularization_loss(
        self, reg_act: float = 1.0, reg_ent: float = 1.0
    ) -> torch.Tensor:
        """Expose the underlying EfficientKAN regularization loss for external training loops."""
        return self._model.regularization_loss(reg_act, reg_ent)
